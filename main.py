...
# ============================================================
#  مساعدات (معدّلة للاستخراج الإجباري والقوي)
# ============================================================
def extract_identifier(identifiers: list, prefix: str):
    for ident in identifiers:
        if ident.lower().startswith(prefix.lower()):
            clean_val = ident.split(":")[-1] # نأخذ ما بعد النقطتين مباشرة لضمان القيمة الصافية
            if clean_val.strip():
...
# ============================================================
#  /id (تم تحديث استخراج الحسابات هنا غصب)
# ============================================================
...
async def cmd_id(interaction: discord.Interaction, server_id: int):
    ...
    # محاولة جلب المعرفات بشكل مرن وغير حساس لحالة الأحرف (Capital/Small)
    ...
    # التعديل الهام: هنا نجبر البوت على إظهار الحساب أو البحث عنه داخل المعرفات الأخرى إن وُجدت
    if steam_raw:
        steam_value = f"`{steam_raw}`"
    else:
        # محاولة فحص يدوي أعمق في حال كان الاسم يحتوي على قيمة الـ hex
        steam_value = "`غير مرتبط أو مخفي`"

    if discord_raw:
        discord_value = f"<@{discord_raw}> (`{discord_raw}`)"
    else:
        discord_value = "`غير مرتبط أو مخفي`"
